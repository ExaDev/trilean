 ...
 *
 * Scope, deliberately: this proves conversion from source is faithful, needing no `pnpm build`. It does not and cannot check the artefact that actually ships -- scripts/generate-json-schema.mjs splices zod's multi-document registry output into one self-contained file, and only a real build produces that. test/smoke.test.mjs owns that half, asserting the generated schemas/json-operators.schema.json resolves all its own $refs and carries exactly the built package's own discriminant set. Re-deriving the script's splicing here instead would only compare the test's copy of that logic against itself.
 *
