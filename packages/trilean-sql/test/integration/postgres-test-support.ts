...
/**
 * The shared harness the split `postgres.*.test.ts` files below all import: the container lifecycle, the seeded rows, the resolver bridging a row to the evaluator's own notion of "known", and the `agreeingRows` comparison every case in those files is built on.
 *
 * Splitting the original single file by tested concern (predicates, edge cases, quantifiers) is purely a file-size matter -- vitest still gives each split file its own isolated module instance, so each one starts, seeds, and stops its own container via the `beforeAll`/`afterAll` registered here, exactly as the unsplit file did for itself.
 */
...
/**
 * Every subject's own tags, seeded to exercise a distinct shape of participation each: `ada` has none (the empty-collection case); `grace`'s two weights (3, 9) straddle the `[RANGE_LOW, RANGE_HIGH]` window used in `postgres.quantifiers.test.ts` without either one falling inside it, and one alone fails `MATCH_THRESHOLD` while the other passes; `lin` pairs one clean, participating vote with one whose `weight` is unknown; `unknown` has a single tag whose `weight` is unknown, the sole-participant indeterminate case.
 */
