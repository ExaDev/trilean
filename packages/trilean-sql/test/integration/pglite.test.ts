 ...
 *
 * PGlite is PostgreSQL itself compiled to WebAssembly and run in this process, not a reimplementation of it or a dialect of its own, so the compiler needs no `dialect` value for it: every construct emitted under `dialect: "postgres"` -- `$N::type` casts, `~`/`!~`, double-quoted identifiers, `NULL::boolean` -- is parsed by the same parser and evaluated by the same planner. That is the claim this file exists to establish rather than assume, which is why it is a full parity suite rather than a smoke test: each case compiles a tree, executes the fragment as a real `WHERE` clause, and compares the rows against the ones trilean's own evaluator judges `definite(true)` for the same tree, exactly as the container-backed suite does.
 *
 * Keeping it a near-verbatim structural copy is deliberate. A shared harness parameterised over both engines would make the two suites agree by construction, and agreement by construction is the one thing this cannot establish -- the point is that two independently-driven executions of the same compiled SQL reach the same rows. The divergence between the files is therefore confined to how a connection is opened and a statement is run.
 *
 * It also needs no Docker daemon, so unlike the container-backed suite it runs anywhere Node does.
 */
...
describe("degenerate and adversarial fragments", () => {
  ...
  it("refuses NaN, and measures the divergence that refusal exists to prevent", async () => {
    // The refusal is in the guard, so this could have been a unit test -- but a unit test could only assert that NaN is refused, not that refusing it was right. What makes it right is a fact about PostgreSQL: it defines NaN as equal to itself, so `NaN = NaN` there is TRUE and would have selected the whole table, while trilean's `===` makes the same tree match nothing. Both halves are measured here, and the first of them is a fact about PGlite worth measuring separately from the server: a WASM build could in principle have shipped a different float comparison, and a driver that could not bind NaN at all -- as better-sqlite3 cannot -- would substitute NULL and produce the opposite row set rather than the same one.
    const node: PredicateNode = {
      kind: "compare",
      op: "eq",
    ...
    const wouldHaveMatched = await db.query<{ id: string }>(
