...
// A small mapping local to this file's own compile-level assertions, distinct from the integration suites' `subject_tags` fixture -- these cases assert exact compiled text rather than measuring against a real connection, so they need no seeded data, only a table/join/columnFor shape to compile against.
