...
/** Variation 2: `x` is missing from the data. `and` has no rescuing value on this side: the left operand (`isActive is true`) is definitely `true`, not `false`, so it cannot absorb the right operand's indeterminacy the way variation 1's `false` does above. */
